...
const DeploymentDetails = () => {
    ...
    useEffect(() => {
        ...
        const handleDeploymentLog = (data) => {
            ...
            if (data.deploymentId && data.deploymentId !== id) {
                return;
            }
            setDeployment((previous) => {
                ...
                return {
                    ...previous,
                    logs: data.log ? ((previous.logs || "") + data.log) : previous.logs,
                    status: data.status || previous.status,
                    currentStep: data.currentStep || previous.currentStep,
                    url: data.url !== undefined ? data.url : previous.url
                };
            });
        };
        ...
        const pollingInterval = setInterval(async () => {
            try {
                ...
                if (latest.status === "success" ||  latest.status === "failed") {
                    ...
                    console.error("Deployment polling failed:", error);
            }
        },2000);
        return () => {
