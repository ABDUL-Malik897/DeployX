...
const githubWebhook = async (req, res) => {
    try {
        ...
        const project = await Project.findOne({ githubFullName: repository });
        if (!project) {
            return res.status(200).json({
...
const updateDeploymentFromGitHub = async (req, res) => {
    try {
        ...
        const expectedSecret = process.env.DEPLOYX_DEPLOYMENT_SECRET;
        const receivedSecret = req.headers["x-deployx-secret"];
        if (!expectedSecret || !receivedSecret || receivedSecret !== expectedSecret) {
            return res.status(401).json({
                message: "Invalid deployment callback secret"
            });
        }

        const { deploymentId, status, url, logs, currentStep, outputDirectory } = req.body || {};
        if (!deploymentId) {
            return res.status(400).json({
        ...
        if (status === "success" && deployment.url) {
        ...
        try {
            ...
            console.error("Deployment socket update failed:", socketError);
        }
        return res.status(200).json({
        ...
        console.error("GitHub deployment callback error:", error);
        return res.status(500).json({
